...
export const JOURNEY_STAGES = [
  ...
  },
  {
    ...
    lessons: [
      ...
      { id: 'skyscraper', name: 'Skyscraper', summary: '兩組強連結共享一端方向，另一端共同可見處可排除。', cue: '找同一數字的兩條雙候選線。' },
      { id: 'kite', name: '2-String Kite', summary: '一個行強連結與一個列強連結透過同宮相接。', cue: '行、列各一條強連結。' },
      { id: 'emptyRectangle', name: 'Empty Rectangle', summary: '利用宮內候選的特殊分布串接行列強連結。', cue: '先辨認宮內的 L 形候選。' }
    ]
  },
  {
    ...
    lessons: [
      ...
      { id: 'xyzWing', name: 'XYZ-Wing', summary: 'XYZ 樞紐連接 XZ、YZ 兩翼，排除三格共同可見的 Z。', cue: '樞紐是三候選格。' },
      { id: 'wWing', name: 'W-Wing', summary: '兩個相同雙候選格，透過其中一數的強連結排除另一數。', cue: '兩翼不必互相看見。' },
      { id: 'simpleColoring', name: '簡單著色', summary: '沿同一數字的強連結交替著色，利用矛盾或陷阱排除。', cue: '全程只追一個數。' },
      { id: 'xChain', name: 'X-Chain', summary: '同一數字的強弱連結交替構成推理。', cue: '先寫清楚每條連結類型。' },
      { id: 'xyChain', name: 'XY-Chain', summary: '雙候選格串鏈，兩端共同可見處排除共同數。', cue: '每一格承接前一格的一個候選。' },
      { id: 'aic', name: 'AIC 交替推理鏈', summary: '以強弱連結交替表達更一般的鏈與環。', cue: '翼、著色與多種鏈可統一到 AIC。' }
    ]
  },
  {
    ...
    lessons: [
      { id: 'als', name: 'ALS／ALS-XZ', summary: 'n 格含 n+1 個候選，透過受限共同候選連接集合。', cue: '先辨認「差一個就鎖定」的集合。' },
      { id: 'sueDeCoq', name: 'Sue de Coq', summary: '結合宮與行列交界的多個候選集合進行排除。', cue: '先拆成交界與兩側集合。' },
      { id: 'finnedFish', name: 'Finned／Sashimi Fish', summary: '基本魚形多出鰭候選後，在受限制區域排除。', cue: '先確認對應的基本魚形。' },
      { id: 'uniqueRectangle', name: '唯一矩形', summary: '利用題目唯一解假設避免形成雙解矩形。', cue: '這不是只靠數獨基本規則的推論。', caution: true },
      { id: 'bugPlusOne', name: 'BUG+1', summary: '接近所有格皆雙候選時，以唯一性條件處理多出的候選。', cue: '必須先確認 BUG 結構。', caution: true },
      { id: 'forcingChain', name: '強制鏈與強制網', summary: '分別檢查候選真假分支，找出所有分支共同結論。', cue: '保留分支證據，不把試填包裝成直覺。' },
      { id: 'search', name: '搜尋驗證與回溯', analyzer: 'search', summary: '當已實作邏輯不足時驗證解；它是求解保底，不是學習技巧。', cue: '分析報告會明確標示，不混入技巧統計。', caution: true }
    ]
  }
];
